...
const RegistryScreen = ({ classes }) => {
  ...
  const handleSubmit = async () => {
    ...
    const hasErrors = validateFormData(requestBody, setErrors, errors)
    if (hasErrors) return;
    ...
    if (isEdit) {
      ...
      try {
        await registryApi.update(requestBody)
      ...
      <DialogComponent open={!!dialogType} onClose={() => handleClearDialog("")} paperProps={{ maxWidth: 500 }}>
        <Box gap={2} display={"flex"} flexDirection={"column"}>
          ...
          <TextField
            ...
            value={name}
            disabled={isEdit}
            helperText={!!errors?.name && "Config Name is required"}
            error={!!errors?.name}
          />
          <TextField
            label="Registry"
            ...
            value={registry}
            helperText={!!errors?.registry && "Registry is required"}
          ...
          />
          <FormLabel error>Note: For anonymous authentication, fill in the username and password with dummy details</FormLabel>
  ...
  const renderDialog = () => {
    const dialogContent = {
      registryForm: renderRegistryForm(),
