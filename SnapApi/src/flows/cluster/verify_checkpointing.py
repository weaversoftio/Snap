...
async def verify_checkpointing(request: VerifyCheckpointRequest, username: str):
    ...
    """
    try:
        await send_progress(username, {"type": "progress", "progress": 30, "task_name": "Cluster Verification", "message": f"Cluster verification initiated with clusterName: {request.clusterName}"})
        print("************************************************************\n")
        ...
        # await send_progress(username, {"type": "progress", "task_name": "Cluster Verification", "message": f"ANSIBLE_PRIVATE_KEY_FILE set to {key_path} for this session."})

        print(f"ANSIBLE_PRIVATE_KEY_FILE set to {key_path} for this session.")
    
        # Log the command
        await send_progress(username, {"type": "progress", "progress": 60, "task_name": "Cluster Verification", "message": f"Running: ansible-playbook -i {inventory_file} {playbook_path}"})
        ...
        if any(details.values()):
            await send_progress(username, {"type": "progress", "progress": "failed", "task_name": "Cluster Verification", "message": output})
            ...
            await send_progress(username, {"type": "progress", "progress": 100, "task_name": "Cluster Verification", "message": output})
            return CheckClusterResponse(
                success=True,
        ...
        await send_progress(username, {"type": "progress", "progress": "failed", "task_name": "Cluster Verification", "message": f"Cluster Verification failed: {str(e)}"})
