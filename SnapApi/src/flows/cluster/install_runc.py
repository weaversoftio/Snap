...
async def install_runc(request: RunCRequest, username: str):
    ...
    """
    await send_progress(username, {"progress": 25,"task_name": "Installing RunC", "message": f"Installing RunC initiated with clusterName: {request.clusterName}"})
    try:
        ...
        try:
            ...
            await send_progress(username, {"progress": 75,"task_name": "Installing RunC", "message": f"Decoding byte streams"})
            ...
            if process.returncode != 0:
                ...
                if not details:
                    ...
                    await send_progress(username, {"progress": "failed", "task_name": "Installing RunC", "message": message})
                ...
                await send_progress(username, {"progress": "failed", "task_name": "Installing RunC", "message": f"Runc installation failed: {stdout}"})
                return RunCResponse(
                    success=False,
            ...
            await send_progress(username, {"progress": 100, "task_name": "Installing RunC", "message": "Runc installation successful"})
            return RunCResponse(
                success=True,
            ...
            await send_progress(username, {"progress": "failed", "task_name": "Installing RunC", "message": f"An error occurred: {str(e)}"})
            return RunCResponse(
                success=False,
